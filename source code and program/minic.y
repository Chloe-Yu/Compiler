...
//% token 定义终结符的语义值类型
%token <type_int> INT CHAR           //指定INT的语义值是type_int，有词法分析得到的数值
%token <type_id> ID RELOP COMPASSIGN TYPE  //指定ID,RELOP 的语义值是type_id，有词法分析得到的标识符字符串
...
%token PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN MINUSONE PLUSONE

%left ASSIGNOP COMPASSIGN
%left OR
%left AND
%left RELOP
%left PLUS MINUS
...
ExtDef:   Specifier ExtDecList SEMI   {$$=mknode(EXT_VAR_DEF,$1,$2,NULL,yylineno);}   //该结点对应一个外部变量声明
         ...
         | error SEMI   {$$=NULL; }
         ;
Specifier:  TYPE    {$$=mknode(TYPE,NULL,NULL,NULL,yylineno);strcpy($$->type_id,$1);$$->type=!strcmp($1,"int")?INT:(strcmp($1,"float")?CHAR:FLOAT);}   
           ;      
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
      | Exp COMPASSIGN Exp {$$=mknode(COMPASSIGN,$1,$3,NULL,yylineno);strcpy($$->type_id,$2);}
      ...
      | PLUSONE Exp %prec NOT   {$$=mknode(PLUSONE,$2,NULL,NULL,yylineno);strcpy($$->type_id,"PLUSONE");}//前置
      | MINUSONE Exp %prec NOT {$$=mknode(MINUSONE,$2,NULL,NULL,yylineno);strcpy($$->type_id,"MINUSONE");}
      ...
      | CHAR	      {$$=mknode(CHAR,NULL,NULL,NULL,yylineno);$$->type_int=$1,$$->type=CHAR;}
