...
struct node * mknode(int kind,struct node *first,struct node *second, struct node *third,int pos ) {
  ...
  T->ptr[1]=second;
  T->ptr[2]=third;
  if(T->ptr[0]!=NULL) {T->ptr[0]->parent=T;T->ptr[0]->seq_num=0;}
  if(T->ptr[1]!=NULL) {T->ptr[1]->parent=T;T->ptr[1]->seq_num=1;}
  if(T->ptr[2]!=NULL) {T->ptr[2]->parent=T;T->ptr[2]->seq_num=2;}
  T->pos=pos;
  return T;
}
